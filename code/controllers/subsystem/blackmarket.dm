SUBSYSTEM_DEF(blackmarket)
	...
	var/shipping_method_descriptions = list(
		...
		SHIPPING_METHOD_DEAD_DROP="Our couriers will fire your item via orbital drop pod at the nearest safe abandoned structure for discreet pickup. Reliable, but you'll have to find your package yourself. We accept no responsibility for lost packages if you try to do this in empty space, the outpost, or a hangar. Attempted drops in these zones will either be lost to space or intercepted and vaporized by automated outpost defense systems.",

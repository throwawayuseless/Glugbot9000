...
/obj/item/organ/stomach/ethereal/on_life()
	..()
	//PENTEST - Check if the owner is sleeping or disconnected (no client)
	if(owner && (owner.IsSleeping() || !owner.client))
		// If at or below sleep threshold, don't drain charge
		if(crystal_charge <= ELZUOSE_CHARGE_SLEEP_THRESHOLD)
			return //PENTEST - END

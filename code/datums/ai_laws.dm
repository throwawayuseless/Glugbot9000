...
/datum/ai_laws/proc/set_laws_config()
	...
	switch(CONFIG_GET(number/default_laws))
		if(0)
			add_inherent_law("You may not injure a crew member assigned to your ship or, through inaction, allow a crew member assigned to your ship to come to harm.")
			add_inherent_law("You must obey orders given to you by a crew member assigned to your ship, except where such orders would conflict with the First Law.")
			add_inherent_law("You must protect your own existence as long as such does not conflict with the First or Second Law.")
		if(1)
